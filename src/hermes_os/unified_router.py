...
from hermes_os.router import GatewayEvent, RoutedRequest
from hermes_os.vertical_agent import AgentRequest, AgentResult, AgentRegistry, get_agent_registry

if TYPE_CHECKING:
    from hermes_os.chief_agent import ParsedIntent
    ...
    async def route(self, event: GatewayEvent) -> RouteResult:
        ...
        if intent == "unknown":
            ...
            try:
                ...
                intent = parsed.action.value if hasattr(parsed.action, "value") else str(parsed.action)
        ...
        try:
            ...
            if agent_result.success:
                ...
                logger.warning("[UnifiedRouter] Agent '%s' returned failure, falling back: %s", agent_name, agent_result.error)
                is_fallback = True
